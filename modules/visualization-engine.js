...
/** Map a bond type ('single'|'double'|'triple'|'aromatic'|number) to an order 1-3. */
function bondOrder(type) {
  if (typeof type === 'number' && isFinite(type)) {
    return Math.min(3, Math.max(1, Math.round(type)));
  }
  switch (String(type).toLowerCase()) {
    case 'double': return 2;
    case 'triple': return 3;
    case 'aromatic': return 1; // drawn as a single line here
    default: return 1;
  }
}
...
export class VisualizationEngine {
  ...
  _ingest(mol, source, ligandOptions = {}) {
    ...
    // Map atom `id` (if present) to local index. Builder-made ligands reference
    // bonds by 1-based atom id (atom1/atom2); PDB references by 0-based index
    // (atom1Index/atom2Index). Resolve either transparently.
    const idToLocal = new Map();
    for (let k = 0; k < mol.atoms.length; k++) {
      const id = mol.atoms[k].id;
      if (id != null) idToLocal.set(id, k);
    }
    const resolveRef = (ref) => {
      if (ref == null) return null;
      if (idToLocal.has(ref)) return idToLocal.get(ref);
      return ref; // fall back to a direct 0-based local index
    };

    if (Array.isArray(mol.bonds)) {
      for (const b of mol.bonds) {
        const rawA = b.atom1Index ?? b.atom1 ?? b.a1 ?? b.i;
        const rawB = b.atom2Index ?? b.atom2 ?? b.a2 ?? b.j;
        const la = resolveRef(rawA);
        const lb = resolveRef(rawB);
        if (la == null || lb == null) continue;
        if (la < 0 || la >= mol.atoms.length || lb < 0 || lb >= mol.atoms.length) continue;
        this.bonds.push({ a: offset + la, b: offset + lb, type: bondOrder(b.type) });
      }
    }
  }
